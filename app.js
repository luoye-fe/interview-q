...
/*
 * 基本：先用节流函数将在设定时间间隔内的请求收集起来一起执行，获取数据完毕之后用广播机制告知每个调用者相对应的数据
 * 进一步：本地缓存一份已知的 profile list，设定缓存时间，缓存时间内不发起真实请求，从本地取
 * 再进一步：ES6 来实现
 * 再再进一步：模块化，eventBus 和 cache 可以封装起来，最后 export 一个函数，调用的时候 import 一个函数即可（为了在浏览器中直观操作，未实现）
 */
...
// 帮助函数，执行向队列添加 id，监听请求完成操作
function getUserProfileHelper(id) {
    return new Promise((resolve, reject) => {
        // 向队列中添加 id
...
function getUserProfile(id) {
    return getUserProfileHelper(id);
}
